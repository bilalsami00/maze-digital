...
//     <div className="absolute left-[60%] top-[450px] transform -translate-x-[60%] sm:left-1/2 sm:-translate-x-1/2 w-[90%] max-w-[580px] min-h-[416px]">
//       <div
//         ref={modalRef}
...
///////////////  12/3/25 trying to fix larger value ////////////////
  ...
  return (
    ...
    <div className="absolute left-[60%] top-[450px] transform -translate-x-[60%] sm:-translate-x-1/2 w-[90%] max-w-[580px] min-h-[416px]">
      ...
      >
        ...
        {/* Details Section - Properly Spaced & Aligned */}
        <div className="text-[#1F1F1F] mt-4 grid grid-cols-2 gap-x-16 gap-y-3 text-[18px] leading-[32px] font-[500] w-[90%] max-w-[500px] mx-auto">
          {/* Left Column */}
          <div className="flex flex-col min-w-0">
            {/* Name Field - Hover + Tap Support */}
            <div
              className="relative group"
              onClick={(e) => e.currentTarget.classList.toggle("show-tooltip")} // Enable tap to show on mobile
            >
              <p className="m-[0.15rem] break-words truncate sm:whitespace-nowrap w-full">
                <strong>Name:</strong> {lead.name}
              </p>

              {/* Tooltip - Visible on hover (large screens) and tap (small screens) */}
              <span className="absolute bottom-full left-0 mb-1 hidden w-max max-w-xs bg-black text-white text-xs rounded px-2 py-1 z-10 group-hover:block show-tooltip:block">
                {lead.name}
              </span>
            </div>

            {/* Email Field - Hover + Tap Support */}
            <div
              className="relative group"
              onClick={(e) => e.currentTarget.classList.toggle("show-tooltip")} // Enable tap to show on mobile
            >
          ...
          <div className="flex flex-col min-w-0">
            ...
            >
              ...
              <span className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 hidden w-max max-w-[90vw] bg-black text-white text-xs rounded px-2 py-1 z-10 group-hover:block show-tooltip:block whitespace-normal break-words text-center">
                {lead.phone}
              </span>
            </div>

            {/* Date Field - Responsive Tooltip */}
            <div
              className="relative group"
              onClick={(e) => e.currentTarget.classList.toggle("show-tooltip")} // Enable tap to show on mobile
            >
              ...
              {/* Tooltip - Visible on hover (large screens) and tap (small screens) */}
              <span className="absolute bottom-full left-0 mb-1 hidden w-max max-w-xs bg-black text-white text-xs rounded px-2 py-1 z-10 group-hover:block">
                {lead.date}
              </span>
            </div>
          </div>
        ...
        {/* Assign To Section */}
        <div className="mt-4 w-[90%] max-w-[500px] mx-auto flex items-center gap-2 flex-nowrap min-w-0">
          {/* Assign To Text */}
          <div className="bg-white rounded-[16px] px-3 py-1 sm:px-4 sm:py-2 border border-gray-300  w-auto min-w-0 overflow-auto ">
            <p className="text-[#1F1F1F] text-[16px] sm:text-[18px] leading-[28px] sm:leading-[32px] font-[500]  w-full">
              <strong>Assign To:</strong> {lead.assignTo || "Not Assigned"}
            </p>
          </div>

          {/* Arrows Container (Prevents Disappearance) */}
          <div className="flex gap-1 shrink-0">
            <button className="w-[20px] h-[20px] flex items-center bg-yellow-400 rounded-full justify-center shrink-0">
              <Image
                src="/images/left-arrow-8x.png"
                alt="Back Arrow"
                width={7}
